...
{
std::cout<<"_onnx_path: "<<_onnx_path<<std::endl;
#ifdef LITE_WIN32
...
void BasicOrtHandler::initialize_handler()
{
  ...
  input_node_dims.at(0) = batch_size;
  
  for (unsigned int i = 0; i < input_node_dims.size(); ++i) {
    input_tensor_size *= input_node_dims.at(i);
    std::cout<<"input_node_dim_"<<i<<": "<<input_node_dims.at(i)<<std::endl;
  }
  
  std::cout<<"handler: input_tensor_size: "<<input_tensor_size<<std::endl;
